...
from models.mymodel_motif import Lucky
...
def read_file(data_type, file_index):
    datas = pd.read_csv(f"data/motif_data/{data_type}/{file_index}-1.csv")
    seq =  list(datas['data'])
...
plt.grid(True)
plt.savefig(f'motif/data10/Importance_Score.svg')
plt.show()
plt.close()
# 取平均（如果需要）
...
plt.tight_layout()
# 保存图像
plt.savefig(f'motif/data10/motifs_combined.svg')
# 显示图像
plt.show()
