...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose1d(in_channels=64, out_channels=512, kernel_size=4, stride=2,
                                          padding=1)

        # 第二层反卷积：进一步恢复序列长度
        self.deconv2 = nn.ConvTranspose1d(in_channels=512, out_channels=512 // 2, kernel_size=4,
                                          stride=2, padding=1)

        # 第三层反卷积：输出重构的 DNA 序列
        self.deconv3 = nn.ConvTranspose1d(in_channels=512 // 2, out_channels=4, kernel_size=4,
                                          stride=3, padding=2)
...
class Lucky(nn.Module):
    ...
    def forward(self, x):
        ...
        # return out, vq_loss, x_recon, perplexity
        return out, vq_loss, x_recon, perplexity, x_final
