...
from ablation.withoutTF import Lucky
...
def to_log(log, params):
    with open(f"../results/ablation/withoutTF/data{params['data_index']}/seed{params['seed']}.log", "a+") as f:
        f.write(log + '\n')
...
def train_model(train_loader, valid_loader, test_loader, params):
    ...
    for epoch in range(params['epoch']):
        ...
        for seq, label in train_loader:
            ...
            logits, vq_loss, data_recon, perplexity = model(seq)
        ...
        if valid_acc > best_acc:
            ...
            torch.save(checkpoint, f"../save/ablation/withoutTF/data{params['data_index']}/seed{params['seed']}.pth")
...
def evaluate(data_iter, net):
    ...
    for j, (data, labels) in enumerate(data_iter, 0):
        ...
        data = data.to(device)
        output, vq_loss, data_recon, perplexity = net(data)
...
def main():
    ...
    for index in range(8, 9):
        params = {
            ...
            'data_index': index,
            'patience': 30,
            'loss1': 0.789661193564039,
            'loss2': 0.8176202811413239,
            'loss3': 0.031021878578579104
        }
