...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # 第一层卷积：将 DNA 序列的输入从 4 个通道扩展到 256 个特征通道
        self.conv1 = nn.Conv1d(in_channels=4, out_channels=256, kernel_size=4, stride=3, padding=2)

        # 第二层卷积：将特征通道从 256 扩展到 64，进一步减小序列长度
        self.conv2 = nn.Conv1d(in_channels=256, out_channels=64, kernel_size=4, stride=2, padding=1)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # 第一层反卷积：将特征通道从 64 扩展到 256
        self.deconv1 = nn.ConvTranspose1d(in_channels=64, out_channels=256, kernel_size=4, stride=2, padding=1)

        # 第二层反卷积：将特征通道从 256 扩展到 4 个通道（恢复到原始输入通道）
        self.deconv2 = nn.ConvTranspose1d(in_channels=256, out_channels=4, kernel_size=4, stride=3, padding=2)

    def forward(self, x):
        x = F.relu(self.deconv1(x))  # 第一层反卷积
