...
class Lucky(nn.Module):
    def __init__(self):
        ...
        #     nn.Conv1d(4, self.kernel_num, kernel_size=3, padding=1),
        #     nn.BatchNorm1d(self.kernel_num),
        #     nn.SiLU(inplace=False),
        #     ResidualStack(in_channels=self.kernel_num,
        #                                          num_hiddens=self.kernel_num,
        #                                          num_residual_layers=2,
        #                                          num_residual_hiddens=32),
        #     nn.Conv1d(self.kernel_num, self.kernel_num, stride=3, kernel_size=7, padding=2),
        #     nn.BatchNorm1d(self.kernel_num),
        #
        # )
        ...
        #self._encoder = Encoder(in_channels=4, num_hiddens=self.kernel_num, num_residual_layers=2, num_residual_hiddens=32)
        # self._residual_stack = ResidualStack(in_channels=self.kernel_num,
        #                                      num_hiddens=self.kernel_num,
        #                                      num_residual_layers=2,
        #                                      num_residual_hiddens=32)
        self._residual_stack2 = ResidualStack(in_channels=256,
                                             ...
                                             num_residual_layers=2,
                                             num_residual_hiddens=32)
        #self._pre_vq_conv = nn.Conv1d(in_channels=self.kernel_num, out_channels=256, kernel_size=1, stride=1)
        self._encoder = Encoder()
        self._vq_vae = VectorQuantizer(num_embeddings=512, embedding_dim=64, commitment_cost=0.25)
        self._decoder = Decoder()
        # self._decoder = nn.Sequential(
        #     nn.ConvTranspose1d(256, self.kernel_num, stride=3, kernel_size=7, padding=2),
        #     nn.BatchNorm1d(self.kernel_num),
        #     nn.SiLU(inplace=False),
        #     ResidualStack(in_channels=self.kernel_num,
        #                   num_hiddens=self.kernel_num,
        #                   num_residual_layers=2,
        #                   num_residual_hiddens=32),
        #     nn.ConvTranspose1d(self.kernel_num, 4, kernel_size=3, padding=1),
        #     nn.BatchNorm1d(4),
        # )
        #self._decoder = Decoder(in_channels=64, num_hiddens=self.kernel_num, num_residual_layers=2, num_residual_hiddens=32)

        # self._encoder = Encoder(1, num_hiddens=128, num_residual_layers=2, num_residual_hiddens=32)
        # self._pre_vq_conv = nn.Conv2d(in_channels=128,
        #                               out_channels=64,
        #                               kernel_size=1, stride=1)
        # self._vq_vae = VectorQuantizer(num_embeddings=512, embedding_dim=64, commitment_cost=0.25)
        # self._decoder = Decoder(in_channels=64, num_hiddens=128, num_residual_layers=2, num_residual_hiddens=32)
        # self.final_large = nn.Sequential(
        #     nn.Linear(256 * 167, 256),
        #     nn.LayerNorm(256),
        #     nn.SiLU(),
        #     nn.Linear(256, 2),
        #     # nn.Softplus()
    ...
    def forward(self, x):
        ...
        #word_detect_conv = self._encoder(x.transpose(1, 2).float())
        ...
        # sequence_embedding = nn.embedding(1, 256)
        # word_embedding = torch.concat([sequence_embedding, ord_embedding, ], dim=1)
