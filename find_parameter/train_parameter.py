...
from VQRNA_find_parameter import VQRNA
import nni
...
def to_log(log, params):
    with open(f"../results/parameter/parameter{params['data_index']}.log", "a+") as f:
        f.write(log + '\n')
...
def train_model(train_loader, valid_loader, test_loader, params,data_variance):
    ...
    for epoch in range(params['epoch']):
        ...
        if valid_acc > best_acc:
            ...
            torch.save(checkpoint,f"../save/find_parameter/parameter{params['data_index']}.pth")
        ...
        if now_epoch > patience:
            ...
            break
        # return best_acc
    nni.report_final_result(best_performance[0])
...
def main():
    ...
    tuner_params = nni.get_next_parameter()  # Get the next set of parameters in the search space
    params.update(tuner_params)  # Update parameters
