...
def plot_logo(X_attr, ax, color=None, annotations=None, start=None, end=None,
    ...
    df = pandas.DataFrame(X_attr.T, columns=['A', 'C', 'G', 'U'])
    df.index.name = 'pos'
    ...
    if color is not None:
        alpha = numpy.array(['A', 'C', 'G', 'U'])
        seq = ''.join(alpha[numpy.abs(df.values).argmax(axis=1)])

        # 如果你需要逐字符设置颜色
        color_mapping = {'A': 'red', 'C': 'blue', 'G': 'orange', 'U': 'green'}
        for i, char in enumerate(seq):
            logo.style_single_glyph(i, char, color=color_mapping[char])
...
def plot_pwm(pwm, name=None, alphabet=['A', 'C', 'G', 'U'], eps=1e-7):
