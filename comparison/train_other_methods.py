...
def to_log(log, params):
    with open(f"../results/comparison/{params['model_name']}/data{params['data_index']}/seed{params['seed']}length51.log", "a+") as f:
        f.write(log + '\n')
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.method = nn.Sequential(
            ...
            # nn.Conv1d(4096, 4096, kernel_size=3, padding=1),
            # nn.BatchNorm1d(4096),
            # nn.SiLU(inplace=False),
            nn.Conv1d(16, 32, stride=3, kernel_size=7, padding=2),
            nn.BatchNorm1d(32),
        )
    ...
    def forward(self, x):
       ...
       # !CNN
       # !除Transformer
        x_final = torch.flatten(x, start_dim=1)
        out = self.final(x_final)
       # !除Transfomer

        #return out, None
        return out, None, x_final
# ========================================================================================
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        embedding_dim = 64
        self.embedding = nn.Embedding(7, embedding_dim)

        self.cls_embedding = nn.Embedding(1, embedding_dim)
        self.positional_encoding = PositionalEncoding(embedding_dim, 601)
        self.layers = nn.ModuleList([EncoderLayer(d_model=embedding_dim,
                                                  ffn_hidden=embedding_dim,
                                                  n_head=8,
                                                  drop_prob=0.2)
                                     for _ in range(6)])
        self.final = nn.Sequential(
    ...
    def forward(self, x):
        ...
        sequence_embedding = self.cls_embedding(torch.zeros((x.shape[0], 1),
                                                            dtype=torch.int, device=x.device))
        word_embedding = torch.concat([sequence_embedding, x], dim=1)
        #
...
class LSTM(nn.Module):
    def __init__(self):
        ...
        self.method = nn.LSTM(embedding_dim, embedding_dim, num_layers=2, batch_first=True, bidirectional=False,
                           dropout=0.2)
...
class GRU(nn.Module):
    def __init__(self):
        ...
        # self.method = nn.GRU(embedding_dim, embedding_dim, num_layers=2, batch_first=True, bidirectional=True,
        #                    dropout=0.2)
        self.method = nn.GRU(embedding_dim, embedding_dim, num_layers=2, batch_first=True, bidirectional=False,
                           dropout=0.2)
...
def train_model(train_loader, valid_loader, test_loader, params):
    ...
    for epoch in range(params['epoch']):
        ...
        if valid_acc > best_acc:
            ...
            torch.save(checkpoint, f"../save/comparison/{params['model_name']}/data{params['data_index']}/seed{params['seed']}length51.pth")
...
def main():
    ...
    for index in range(1, 2):
        ...
        print(params['model_name']+" is running")
        seed = params['seed']
