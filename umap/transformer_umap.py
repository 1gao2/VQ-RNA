...
import sys
sys.path.append('../')
from comparison.train_other_methods import Transformer
import umap
...
def load_model_and_data():
    ...
    model.load_state_dict(torch.load(f"../save/comparison/Transformer/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
def test_all_so_far(model, valid_loader):
    """
    示例测试函数，返回特征和标签。
    """
    maps, vlabels = [], []
...
def draw_umap(x, vlabels):
    ...
    plt.title(f'UMAP Visualization', fontsize=15)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    os.makedirs('../results/umap', exist_ok=True)
    index = {params['data_index']}
    plt.savefig(f'../results/umap/data{index}_Transformer_UMAP.svg', format='svg')
    plt.show()
...
def main():
    model, valid_loader = load_model_and_data()
    maps, vlabels = test_all_so_far(model, valid_loader)
