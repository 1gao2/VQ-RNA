...
def load_model_and_data():
    ...
    model.load_state_dict(torch.load(f"../save/ablation/loss/loss1/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
def test_all_so_far(model, train_loader):
    """
    示例测试函数，返回特征和标签。
    """
    maps, vlabels = [], []
...
def draw_umap(x, vlabels):
    ...
    plt.title(f'w/o VQ_loss and recon_loss', fontsize=15)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    os.makedirs('../results/umap', exist_ok=True)
    index = {params['data_index']}
    # plt.savefig(f'../results/umap/data{index}_loss1_UMAP.svg', format='svg')
    plt.savefig(f'../results/umap/data{index}_loss1_UMAP.svg', format='svg')
    # plt.savefig(f'../results/umap/data{index}_loss1+loss3_UMAP.svg', format='svg')
    plt.show()
...
def main():
    ...
    maps, vlabels = test_all_so_far(model, train_loader)
