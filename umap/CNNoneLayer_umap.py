...
import sys
sys.path.append('../')
from ablation.CNN_oneLayer import Lucky
import umap
...
def load_model_and_data():
    ...
    model.load_state_dict(torch.load(f"../save/ablation/CNN_oneLayer/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
def draw_umap(x, vlabels):
    ...
    plt.title(f'Encoder with one_CNNLayer', fontsize=15)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    os.makedirs('../results/umap', exist_ok=True)
    index = {params['data_index']}
    plt.savefig(f'../results/umap/data{index}_CNNoneLayer_UMAP.svg', format='svg')
    plt.show()
