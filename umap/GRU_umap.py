...
import sys
sys.path.append('../')
from comparison.train_other_methods import GRU
import umap
...
def load_model_and_data():
    ...
    # 加载模型
    model = GRU().to(device)
    model.load_state_dict(torch.load(f"../save/comparison/GRU/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
def draw_umap(x, vlabels):
    ...
    plt.savefig(f'../results/umap/data{index}_GRU_UMAP.svg', format='svg')
    plt.show()
